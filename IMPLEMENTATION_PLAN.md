...
parser/navigation reliability 0.5.3 is production-validated. The 0.5.8 PDF import reliability
release is implementation complete with release validation pending. Version 0.5.9 adds conservative
availability detection; Phases 11B–11D are not started.
...
The 0.5.6 PDF import release removes the unsafe generic property fallback. An English
`Your booking is confirmed at …` line is authoritative property evidence, while a card-method
list or a generic payment/cancellation/amenity/tax/guest/contact section is never a property.
English `DD Month YYYY` dates are accepted only from explicit arrival/departure evidence; payment,
cancellation, issuance, and confirmation dates remain non-stay evidence, and conflicts require
manual review. The adults-only `children=0` rule, immutable reservations, and exact/equivalent/
...
The 0.5.8 PDF import release derives hotel identity from a strict canonical Booking hotel link.
Its visible property text must geometrically overlap the PDF annotation after composition of text
and graphics transforms, including Gmail layouts. Query and fragments are discarded; confirmation,
help, payment, homepage, and external links are rejected. Duplicate identical hotel links are
deduplicated, different hotel links require manual review, and the generic arbitrary-line fallback
is removed. The safe confirmation anchor remains a fallback. No migration, stored-reservation
rewrite, or matcher change is required.

Version 0.5.9 adds `availability_unknown` only after a bounded `goto` → wait → scroll → one
allowlisted availability CTA → wait path yields neither offer evidence nor explicit no-availability.
The removed second `goto` is not retried; booking, payment, and confirmation controls are never
activated. CAPTCHA/login and navigation error prevail, while concrete room/rate/current-price hints
continue to parser classification. The state writes no price, delta, `PRICE_DROP`, or
`CHECK_FAILED`, leaves the technical failure series unchanged, and schedules two-hour retry. The
Comfortable and Downtown CTA was live-validated; PDF import, matcher, scheduler lock, and pricing
are unchanged.
...
| PDF import reliability 0.5.8 | IMPLEMENTATION COMPLETE, RELEASE VALIDATION PENDING |
...
### Phase 11B — reservation overview (in progress; planned 0.6.0)
...
### Phase 11C — property images (not started; planned 0.6.1)
...
### Phase 11D — reservation detail and price history (in progress; planned 0.6.2)
