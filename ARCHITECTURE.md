...
the matcher; the local public PDF-upload E2E passed without a migration or stored-reservation
rewrite. Version 0.5.6
...
reservation rewrite. Version 0.5.9 classifies a shell that remains empty after bounded waiting,
scroll, and one allowlisted availability CTA as `availability_unknown`; it never clicks a booking,
payment, or confirmation control or retries the full navigation. CAPTCHA/login, navigation errors,
recognized offers, and explicit no-availability retain priority. Concrete room/rate/current-price
